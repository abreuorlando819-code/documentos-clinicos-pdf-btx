(() => {
  ...
  function setView(name) {
    ...
    Object.entries(views).forEach(([k, el]) => {
      if (!el) return; // evita crash se algum id estiver diferente
  ...
  // Botão NOVO (limpa ficha clínica e mantém data)
  ...
  on("saveSettings", "click", () => {
    if ($("p_nome")) prof.nome = $("p_nome").value.trim();
    if ($("p_conselho")) prof.conselho = $("p_conselho").value.trim();
    if ($("p_tel")) prof.tel = $("p_tel").value.trim();
    prof.cidade = ($("p_cidade")?.value.trim() || "Belém/PA");
    if ($("p_end")) prof.end = $("p_end").value.trim();
    closeModal();
  });
  ...
  on("btnCarregarModeloLaudo", "click", () => {
    ...
    if ($("l_texto")) $("l_texto").value = txt;
  });

  on("btnLimparLaudo", "click", () => {
    if ($("l_texto")) $("l_texto").value = "";
  });
  ...
  // Botões PDF + imprimir (com proteção)
